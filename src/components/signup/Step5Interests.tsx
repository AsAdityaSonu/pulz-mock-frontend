...
import { groupGamesByCategory, renderGameSection, Game as ImportedGame } from './RenderGames';
...
export const Step5Interests: React.FC<Step5InterestsProps> = ({
  ...
  const renderFooter = () => (
    <View className="space-y-4 mt-8">
      ...
      <CustomButton title="Back" variant="outline" onPress={onBack} size="large" />
    </View>
  );
  ...
  return (
    <View className="py-4">
      ...
      {/* Helper Text */}
      <Text className={`text-sm mb-4 text-center ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>
        We'll use your gaming preferences to suggest friends, communities, and content you'll love.
      </Text>

      {/* Games by Category */}
      <View className="mb-4">
        {Object.entries(groupGamesByCategory(games)).map(([category, categoryGames]) =>
          renderGameSection(category, categoryGames, isDark, isSelected, handleGameSelect)
        )}
      </View>
