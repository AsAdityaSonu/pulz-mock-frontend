    ...
    <SafeAreaView edges={['top', 'left', 'right']} className={`flex-1 ${isDark ? 'bg-gray-900' : 'bg-white'}`}>
      ...
      <ScrollableContainer>
        ...
        <View className="items-center py-8">
          ...
          <Text className={`text-lg font-medium mb-2 ${isDark ? 'text-yellow-400' : 'text-yellow-600'}`}>
            Current Streak
          </Text>
              ...
              <View className="flex-row items-center justify-between">
                <View className="flex-row items-center flex-1">
                  <View className={`w-12 h-12 rounded-full items-center justify-center ${
                    item.color === 'blue' ? (isDark ? 'bg-blue-800' : 'bg-blue-100') :
                    item.color === 'green' ? (isDark ? 'bg-green-800' : 'bg-green-100') :
                    (isDark ? 'bg-purple-800' : 'bg-purple-100')
                  }`}>
                    <item.icon size={20} color={
                      item.color === 'blue' ? (isDark ? '#60a5fa' : '#3b82f6') :
                      item.color === 'green' ? (isDark ? '#4ade80' : '#10b981') :
            ...
            {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day, index) => (
              <View key={index} className="items-center">
                ...
                <View className={`w-8 h-8 rounded-full ${
                  index < 3 ? (isDark ? 'bg-green-700' : 'bg-green-200') : 
            ...
            {[
              ...
              <View
                ...
                className={`p-4 rounded-xl flex-row items-center ${
                  achievement.earned ? 
                    (isDark ? 'bg-yellow-900 border border-yellow-700' : 'bg-yellow-50 border border-yellow-200') :
